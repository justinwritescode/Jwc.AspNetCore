...
public class Pager<T> : ArrayResponsePayload<T>, IPayload<T[]>, IPager<T>, IPayload, IPager
{
    ...
    public Pager(T[]? items, int page, int pageSize, int totalRecords, string? message = default, string itemSeparator = ArrayPayload<T>.DefaultItemSeparator) : base(items, message: message, itemSeparator: itemSeparator)
    {
        Page = page;
        PageSize = pageSize;
    ...
    [JProp("items")]
    public virtual T[]? Items { get => Values; set => Value = value; }

    [JIgnore]
    public override T[]? Values { get => base.Values; set => base.Values = value; }
    ...
    {
        get => _statusCode.HasValue && !_statusCode.Value.IsBetween(200, 299) ? _statusCode.Value :
        HasNextPage || Page > 1 ? (int)PartialContent : (int)OK;
        set => _statusCode = value;
    }

    object? IPayload.Value { get => Value; set => Items = (value as IEnumerable ?? Empty<object>())?.OfType<T>().ToArray(); }
    object[]? IPager.Items { get => Items.OfType<object>().ToArray(); set => Items = value.OfType<T>().ToArray(); }

    public static implicit operator Pager<T>(T[]? items) => new(items, 1, items.Length, items.Length);
    ...
    public static new OpenApiSchema GetOpenApiSchema()
    {
        ...
        schema.Properties.Add("items", new OpenApiSchema { Type = "array", Items = new OpenApiSchema { Reference = new OpenApiReference { Id = typeof(T).Name, Type = ReferenceType.Schema } } });
        return schema;
    }
