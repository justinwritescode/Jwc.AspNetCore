...

{
    ...
    [JProp("item")]
    public virtual T? Item { get => (Items ?? new T?[] { default }).FirstOrDefault(); set => Items = new[] { value }!; }

    [JIgnore]
    public override T[]? Items { get => base.Items; set => base.Items = value; }

    [JIgnore]
    public override T[]? Value { get => base.Value; set => base.Value = value; }

    [JIgnore]
    T? IPayload<T>.Value { get => Item; set => Item = value; }
    object[]? IPager.Items { get => (Items ?? new[] { default(T) }).OfType<object>().ToArray(); set => Items = (value ?? new[] { default(object) }).OfType<T>().ToArray(); }
    ...
    public static new OpenApiSchema GetOpenApiSchema()
    {
        ...
        schema.Properties.Add("item", new OpenApiSchema { Reference = new OpenApiReference { Id = typeof(T).Name, Type = ReferenceType.Schema } });
        return schema;
    }

    public static new SingleItemPager<T> NotFound() => new() { StatusCode = (int)HttpStatusCode.NotFound };
    public static new SingleItemPager<T> BadRequest() => new() { StatusCode = (int)HttpStatusCode.BadRequest };
    public static new SingleItemPager<T> NoContent() => new() { StatusCode = (int)HttpStatusCode.NoContent };
}
