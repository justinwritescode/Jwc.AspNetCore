...
public class ApiKeyAuthenticationHandler : AuthenticationHandler<ApiBasicAuthenticationOptions>
{
    ...
    public ApiKeyAuthenticationHandler(IOptionsMonitor<ApiBasicAuthenticationOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock, UserManager<User> userManager) : base(options, logger, encoder, clock)
    {
    ...
    {
        if (Request.Path.StartsWithSegments("/swagger") || Request.Path.StartsWithSegments("/api-docs") || Request.Path.StartsWithSegments("/api-docs.json"))
            return AuthenticateResult.NoResult();
        else if(Request.Path.StartsWithSegments("/api") && Request.Method.Equals(HttpRequestMethodNames.Options, StringComparison.InvariantCultureIgnoreCase))
            return AuthenticateResult.NoResult();
        if (Request.Headers.TryGetValue(HttpRequestHeaderNames.Authorization, out var authHeader))
        {
            var authHeaderVal = Regex.Replace(authHeader.ToString(), "Basic ", "", RegexOptions.IgnoreCase);
            var authHeaderValDecoded = System.Text.Encoding.UTF8.GetString(FromBase64String(authHeaderVal));
            ...
            if (user != null && await _userManager.CheckPasswordAsync(user, authPassword))
            {
                ...
                await Context.SignInAsync(ApiBasicAuthenticationOptions.AuthenticationSchemeName, principal);
